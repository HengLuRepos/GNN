...
for ep in range(cfg.epoch):
    for batch in range(num_batch):
        ...
        for index, user_index in enumerate(users_index):
            ...
            all_items.append(pos_item_user.item())
            neg_item_user = []
        ...
        loss = agent.bpr_loss(users_index, pos_items, neg_items) + agent.batch_cl_loss(users_index, np.array(all_items))
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
